...
<div class="card shadow mb-4 mt-2">
    <div class="card-body">
        <form <?php if($aksi == 'add'){ ?> action="<?= base_url('ahli/simpan_kriteria') ?>" <?php }else{ ?> action="<?= base_url('ahli/update_kriteria/'.$kriteria->id) ?>" <?php } ?>  method="POST">
			<div class="form-group">
				<label for="nama">Nama Kriteria</label>
				<input type="text" readonly placeholder="Masukkan nama kriteria" class="form-control" name="nama_kriteria" <?php if($aksi == 'edit'){ ?> value="<?= $kriteria->nama_kriteria ?>" <?php } ?> id="nama" required> 
			</div>
			<div class="form-group">
			...
			<a href="<?= base_url('ahli/kriteria') ?>" class="btn btn-secondary">Batal</a>
		</form>
    </div>
</div>
